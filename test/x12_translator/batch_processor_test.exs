defmodule X12Translator.BatchProcessorTest do
  use X12Translator.DataCase, async: true

  alias X12Translator.BatchProcessor
  alias X12Translator.BatchProcessor.BatchResult
  ...
  describe "in-memory processing" do
    test "does not create output files" do
      ...
      batches = Repo.all(X12Translator.Conversions.Batch)
      assert length(batches) > 0

      # Jobs should have JSON in database
      jobs = Repo.all(X12Translator.Conversions.Job)
