defmodule X12Translator.RemoteFetcherTest do
  ...
  describe "configuration" do
    test "uses default timeout when not configured" do
      ...
      config = Application.get_env(:x12_translator, :remote_fetcher, [])
  ...
  describe "manual testing helpers" do
    ...
    test "can serve test ZIPs via local HTTP server" do
      ...
      4. Run in IEx:
         {:ok, result} = X12Translator.RemoteFetcher.fetch_and_extract("http://localhost:8000/test_batch_3files.zip")
         X12Translator.RemoteFetcher.cleanup_temp_files(result.temp_dir)
